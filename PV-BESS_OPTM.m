...
NPVt=NPVe+NPVu;   
%show(NPVt);
NPVt_total = sum(NPVt(:)); % This will sum all elements in the NPVt array


%% Objective Function
linprob1 = optimproblem('Objective',NPVt_total);

%objective = sum(NPVt(1,:),'all'); 
%linprob1=optimproblem('Objective',NPVt);
%linprob1=optimproblem;
%linprob1.Objective= NPVt ;
% % A1=sum(NPVt(1,:),'all');
% % A2=sum(NPVt(2,:),'all');
% % A3=sum(NPVt(3,:),'all');
% % AA=[A1;A2;A3];
%linprob1=optimproblem('Objective',NPVt);
%linprob1= @(linprob1_solution) NPVt ;
%%
...
linprob1.Constraints.econstraints1=econstraints1
%show(econstraints1)
...
linprob1.Constraints.econstraints2=econstraints2
%show(econstraints2)
...
linprob1.Constraints.econstraints3 = econstraints3;
%show(econstraints3)
...
for t = 1:T
    for j = 1:Kbs
        if t == 1
            ...
            econstraints3(j, t) = SOC(j, t) == Bess_Cap * SOC_min + (Pbs(j, t) / n_disch) - P_dis(j, t) / n_disch;
        else
            % SOC calculation for the rest of the day
            econstraints3(j, t) = SOC(j, t) == SOC(j, t - 1) + (P_charge(j, t) / n_charg - P_dis(j, t) / n_disch);
        end
    end
end

% Add SOC constraints to the problem
linprob1.Constraints.econstraints4 = econstraints3;
%show(econstraints3)
% Import power constraint (for each unit and time period)
econstraints4 = optimconstr(Kpv, T);

for i = 1:T
    for j = 1:Kpv
       
        econstraints4(j, i) = Pimp(j, i) >= P_load_E(j, i) - Ppv(j, i) - P_dis(j, i);
    end
end

% Add the import power constraints to the optimization problem
linprob1.Constraints.econstraints4 = econstraints4;
%show(econstraints4)
...
for i = 1:T
    for j = 1:Kpv
        ...
        econstraints5(j, i) = Pexp(j, i) >= Ppv(j, i) - P_load_E(j, i) - P_charge(j, i);
    end
end

% Add the export power constraints to the optimization problem
linprob1.Constraints.econstraints5 = econstraints5;
% show(econstraints5)
%%
options = optimoptions('gamultiobj', 'Display', 'iter', 'MaxGenerations', 700, 'PopulationSize', 100, 'PlotFcn', 'gaplotscores');

% Solve the optimization problem with the specified options
[linprob1_solution, fval, exitflag] = gamultiobj(linprob1, 7,[Ppv,Pimp,Pexp,P_dis,P_charge,Pbs,SOC], options);
...
%%
% Assuming you have defined your optimization problem function as myOptimizationFunction

% Define or load your input data (expressionArray, Kpv, T, P_load_E, Ppv, Pimp, Pexp, Pbs)
% ...
...
% % 
% % % Add SOC constraints to the problem
% % prob.Constraints.econstraints4 = econstraints3;
% % %show(econstraints3)
% % % Import power constraint (for each unit and time period)
% % econstraints4 = optimconstr(Kpv, T);
% % 
...
% optima_cost = linprob1_solution.Objective
% PV = linprob1_solution.Power_PV
% BESS = linprob1_solution.Bess_power
% BESS_charging=linprob1_solution.BESS_charge
% BESS_Discharging=linprob1_solution.BESS_discharge
% Import = linprob1_solution.Power_imp
% Export = linprob1_solution.Power_exp